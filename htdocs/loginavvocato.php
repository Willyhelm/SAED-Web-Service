...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email_err) && empty($password_err)) {
        // Prepara la select
        $sql = "SELECT `nome`, `cognome`, `email`, `password` FROM `avvocato` WHERE `email` = :email";
                ...
                if ($stmt->rowCount() == 1) {
                    if ($row = $stmt->fetch()) {
                        ...
                        if (password_verify($password, $hashed_password)) {
                            ...
                            $_SESSION['tipo'] = 'avvocato';

                            // Reindirizza al profilo
                            header('Location: profiloavvocato.php', true, 302);
                            exit(0);
                        } else {
...
<html lang="it">
  <head>
    ...
    <title>Accesso avvocati - Studio Legale L. & S.</title>
    <!-- Bootstrap core CSS -->
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css">
  ...
  <header class="masthead mb-auto">
    ...
    <h3 class="masthead-brand"><a class="navbar-brand" href="index.php">
      ...
      <nav class="nav nav-masthead justify-content-center">
        ...
        <p class="nav-link active">Accesso avvocati</p>
      </nav>
    </div>
  </header>
  ...
  <div class="wrapper">
        <h2>Accedi come avvocato</h2>
        <p>Benvenuto, avvocato.<br>Inserisci le credenziali d'accesso.</p>
        <div class="frm">
          <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
              ...
              <div>Sei un cliente?<a href="login.php" class="btn btn-link">Accedi qui</a></div>
          </form>
        </div>
    </div>
  </main>
